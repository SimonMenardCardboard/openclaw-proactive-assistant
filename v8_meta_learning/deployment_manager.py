...
from deployment_manager_alias import add_shell_alias
...
class DeploymentManager:
    ...
    def deploy_optimization(self,
        ...
        is_llm_generated = generated_code.get('generated_by') == 'llm'
        
        # Step 2: Check if auto-approved
        # LLM-generated code NEVER auto-approves (security)
        auto_approved = (sandbox_passed and 
                        confidence >= self.auto_approve_threshold and
                        not is_llm_generated)
        ...
        if auto_approved:
            ...
            print(f"🤖 LLM-generated code requires manual approval")
    ...
    def _deploy_approved(self, proposal_id: int, generated_code: Dict) -> Dict:
        ...
        if result['success']:
            ...
            # Add shell alias for command retry wrappers
            if generated_code.get('language') == 'bash' and 'retry' in script_name:
                add_shell_alias(script_name, deployment_path)
