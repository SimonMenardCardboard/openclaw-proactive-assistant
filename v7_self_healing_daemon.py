...
try:
    ...
    from integrations.intelligence.v7_system_health_monitor import SystemHealthMonitor, HealthAlert
    from integrations.intelligence.v7_dependency_graph import DependencyGraph
    from integrations.intelligence.v7_auto_diagnosis import AutoDiagnosis
    from integrations.intelligence.v7_self_repair import SelfRepair

# PERFORMANCE FIX: Use dependency injection to prevent duplicate instances
try:
    from dependency_injection import ServiceContainer
    DI_AVAILABLE = True
except ImportError:
    try:
        from integrations.intelligence.dependency_injection import ServiceContainer
        DI_AVAILABLE = True
    except ImportError:
        DI_AVAILABLE = False
...
class SelfHealingDaemon:
    ...
    def __init__(self, workspace: Optional[Path] = None, dry_run: bool = False, check_interval: int = 60):
        ...
        # PERFORMANCE FIX: Use dependency injection container
        # This prevents creating duplicate instances of SystemHealthMonitor
        if DI_AVAILABLE:
            self.container = ServiceContainer(workspace)
            self.health_monitor = self.container.get_system_health_monitor()
            self.diagnosis_engine = self.container.get_auto_diagnosis()
            self.repair_engine = self.container.get_self_repair()
            # Note: Diagnosis and Repair now share the same health_monitor instance!
        else:
            # Fallback: Old way (creates duplicates)
        ...
        # Dependency graph doesn't need injection (stateless)
        self.dependency_graph = DependencyGraph(workspace)
        
        # File descriptor monitor (NEW - V7 enhancement)
        try:
            from v7_fd_monitor import FileDescriptorMonitor
            self.fd_monitor = FileDescriptorMonitor(self.thresholds)
        except ImportError:
            self.fd_monitor = None
    ...
    def _load_thresholds(self) -> dict:
        ...
        config_file = self.workspace / "integrations/intelligence/v7_threshold_config.json"
        
        # Default thresholds (conservative)
        defaults = {
            "oauth_monitor": {
                "expiry_warning_hours": 168,
                "expiry_critical_hours": 24,
                "auto_refresh": False
            },
            "log_freshness": {
                "warning_threshold_hours": 168,
                "critical_threshold_hours": 336
            },
            "file_descriptors": {
                "warning_threshold": 500,
                "critical_threshold": 1000
            },
            "auto_repair_whitelist": [],
            "approval_required": ["*"]
        }
        
        if not config_file.exists():
            print(f"[{datetime.now().isoformat()}] ⚠️  Config not found: {config_file}", flush=True)
            print(f"  Using default thresholds", flush=True)
            return defaults
        
        try:
            with open(config_file) as f:
                config = json.load(f)
            print(f"[{datetime.now().isoformat()}] ✅ Loaded threshold config", flush=True)
            print(f"  OAuth warning: {config.get('oauth_monitor', {}).get('expiry_warning_hours', 'N/A')}h", flush=True)
            print(f"  Log freshness: {config.get('log_freshness', {}).get('warning_threshold_hours', 'N/A')}h", flush=True)
            print(f"  FD threshold: {config.get('file_descriptors', {}).get('warning_threshold', 'N/A')}", flush=True)
            return config
        except Exception as e:
            print(f"[{datetime.now().isoformat()}] ❌ Failed to load config: {e}", flush=True)
            print(f"  Using default thresholds", flush=True)
            return defaults
    ...
    def start(self):
        ...
        # Use async event loop for non-blocking sleep
    ...
    async def _async_run(self):
        """Async main loop with non-blocking sleep."""
        while self.running:
            try:
                self._health_check_cycle()
                
                if self.running:
                    # Non-blocking sleep - daemon can respond to signals faster
    ...
    def _health_check_cycle(self):
        ...
        # Check file descriptors (NEW - V7 enhancement)
        if self.fd_monitor:
            fd_results = self.fd_monitor.check_all()
            fd_warnings = {name: result for name, result in fd_results.items() if result.get('status') in ['warning', 'critical']}
            
            if fd_warnings:
                print(f"  ⚠️  File descriptor warnings:", flush=True)
                for name, result in fd_warnings.items():
                    print(f"    - {name}: {result.get('fd_count')} FDs (threshold: {result.get('threshold')})", flush=True)
                    
                    # Auto-restart if critical
                    if result.get('status') == 'critical' and result.get('action') == 'restart_process':
                        action = result.get('action')
                        if action in self.thresholds.get('auto_repair_whitelist', []):
                            print(f"    ✅ Auto-repairing: {action} for {name}", flush=True)
                            # TODO: Implement restart logic
                        else:
                            print(f"    ⏸️  Repair needs approval: {action}", flush=True)
