...
class DesktopObserverV2:
    ...
    def get_recent_activities(self, days: int = 7, device_id: int = None) -> List[Dict[str, Any]]:
        """Get recent activity observations for pattern detection.
        ...
        Returns:
            List of activity observations with application and action data
        """
        try:
            conn = sqlite3.connect(str(self.observer_db))
            cursor = conn.cursor()
            ...
            if device_id:
                cursor.execute('''
                    SELECT timestamp, app_name, action_type, window_title, metadata
