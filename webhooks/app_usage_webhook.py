#!/usr/bin/env python3
"""
App Usage Webhook - Desktop Activity Tracking

Receives app usage data from desktop agents (macOS/Windows/Linux).
Stores in SQLite database for V8 pattern learning.

API Endpoints:
- POST /api/usage/desktop - Receive desktop app usage
...
DB_PATH = WORKSPACE / 'transmogrifier' / 'openclaw-proactive-assistant' / 'app_usage' / 'desktop_usage.db'
...
def init_database():
    ...
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_desktop_usage_user_timestamp 
        ON desktop_usage(user_id, timestamp)
    ''')
    
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_desktop_usage_app 
        ON desktop_usage(app_name)
    ''')
...
def record_usage(user_id: str, activities: List[Dict]) -> bool:
    ...
    try:
        for activity in activities:
            cursor.execute('''
                INSERT INTO desktop_usage (user_id, app_name, duration_seconds, platform, timestamp)
                VALUES (?, ?, ?, ?, ?)
            ''', (
                user_id,
                activity['app_name'],
...
@app.route('/api/usage/desktop', methods=['POST'])
@require_auth
def record_desktop_usage():
    """
    Receive desktop app usage data
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='App Usage Webhook Server')
